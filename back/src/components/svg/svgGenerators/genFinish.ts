...
export function genFinishDoorSvg(width: number, height: number, colorOrImage?: { imageUrl?: string; fallbackColor?: string }): SVGSVGElement {
  // 1. 타입 결정
  let type: "vertical" | "square" | "horizontal";
  if (height > width) type = "vertical";
  else if (height === width) type = "square";
  ...
  let fillValue: string = colorOrImage?.fallbackColor || "#eee";
  ...
  let mainShape: SVGRectElement | SVGPathElement;
  if (shape.type === "rect") {
    mainShape = document.createElementNS(SVG_NS, "rect");
    const rectShape = shape as { type: "rect"; x: number; y: number; width: number; height: number };
    ...
    const pathShape = shape as { type: "path"; d: string };
