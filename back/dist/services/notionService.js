...
exports.createNotionOrderPage = createNotionOrderPage;
...
async function createNotionOrderPage(payload) {
    ...
    function interpretOptions(opts, orderType) {
        if (!opts)
            return "-";
        if (orderType === "DELIVERY") {
            // recipient_road_address, delivery_type, delivery_request 등 상세 표시
            const lines = [];
            if (opts.recipient_road_address)
                lines.push(`배송 주소: ${opts.recipient_road_address}`);
            if (opts.delivery_type)
                lines.push(`희망 도착: ${opts.delivery_type === "TOMORROW" ? "내일/지정일" : "오늘중"}`);
    ...
    const deliveryInfoBlock = {
        ...
        paragraph: {
            rich_text: [
                { type: "text", text: { content: `${productTypesStats}\n
...
` } }
    ...
    async function makeFurnitureBlock(item, i) {
        ...
        const textContent = `${i + 1}. [${PRODUCT_TYPE_LABEL[item.product_type]}]
세부종류: \n${optionStr}
개수: ${item.item_count}
...
`;
    ...
    const furnitureBlocks = await Promise.all(payload.orderItems.map((item, i) => makeFurnitureBlock(item, i)));
