// genMaeda.ts
import { MAEDA_DOOR_RECTS, MaedaSubtype } from "../svgData/maedaDoorData";
import { makeRect } from "../svgCore/svgUtils";
import { ColorProps, Size, DoorDimensions } from "../svgCore/svgTypes";
...
const MAEDA_DIMENSION_POSITIONS: Record<MaedaSubtype, DoorDimensions> = {
  '마에다1': {
    verticalPos: {
...
export function genMaedaDoorSvg(
  size: Size,
  color: ColorProps = {}
): SVGSVGElement {
  // 가로세로 길이에 따라 subtype 결정
  let subtype: MaedaSubtype;
  if (size.height > size.width) subtype = '마에다1';
  else if (size.height === size.width) subtype = '마에다2';
  else subtype = '마에다3';
...
function insertDimensionTexts(svg: SVGSVGElement, subtype: MaedaSubtype, size: Size) {
