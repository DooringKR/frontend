...
const Button2: React.FC<ButtonProps> = ({ type, text, onClick, disabled, icon }) => {
    const getClassName = () => {
        switch (type) {
            case 'Brand':
                return [
    ...
    return (
        <button
            className={`button w-full ${getClassName()}`}
            onClick={onClick}
            disabled={disabled}
        >
            <div className='flex gap-1 items-center justify-center'>
                {icon && (
                    <span
                        className={`icon-left ${disabled ? 'text-gray-300' : 'text-gray-700'}`}
                    >
                        {icon}
                    </span>
                )}
