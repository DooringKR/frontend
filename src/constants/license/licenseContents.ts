// constants/licenseContents.ts
export const LICENSE_CONTENTS: Record<string, string> = {
  tossface: `
토스페이스 라이선스 전문
정의
...
허가 및 조건
1. 토스페이스는 그 자체를 판매하거나, 부정한 목적으로 이용하거나, 허용되지 않은 수정본을 만들지 않는 한 자유롭게 사용, 연구, 재배포 하실 수 있습니다.  
2. 토스페이스의 원본은 ‘토스페이스 저작권 안내’와 본 라이선스 전문에 대한 내용을 포함하는 경우에는 다른 소프트웨어와 번들되거나 재배포가 가능합니다. 이는 별도 텍스트 파일과 가독성이 있는 헤더, 또는 (사용자가 쉽게 읽을 수 있다면) 기계가 읽을 수 있는 텍스트나 바이너리 파일의 메타데이터 필드 형태로도 가능합니다.  
3. 토스페이스의 저작권자 혹은 저자의 이름은 그들의 명시적 서면 허가가 있거나 또는 그들의 공헌을 인정하기 위한 경우를 제외하고는 수정본에 대한 사용을 유도, 추천 혹은 광고하기 위한 목적으로 사용할 수 없습니다.
