...
async function getOrderItemFromBackend(orderItemId: number) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("❌ 백엔드 주문 품 조회 실패:", response.status, response.statusText, errorText);
            throw new Error(`주문 품 조회 실패: ${response.status} ${response.statusText} - ${errorText}`);
        }

        const data = await response.json();
        console.log("✅ 백엔드 주문 품 조회 성공:", data);
...
export async function GET(
    ...
    console.log("🚀 /api/order_item/[order_item_id] GET 요청 시작");
    ...
    try {
        ...
        if (!order_item_id) {
            console.log("❌ order_item_id 파라미터가 없음");
        ...
        if (isNaN(orderItemIdNumber)) {
            console.log("❌ 유효하지 않은 order_item_id");
            return NextResponse.json(
