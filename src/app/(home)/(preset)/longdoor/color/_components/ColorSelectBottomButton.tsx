...
const ColorSelectBottomButton: React.FC<ColorSelectBottomButtonProps> = ({ selectedColor, onClick, type }) => {
    return (
        <BottomButton
            children={
                selectedColor && (
                    <div className="flex items-center justify-center gap-2 bg-white px-5 pb-4 pt-2">
                        <Image
                            src={DOOR_COLOR_LIST.find(item => item.name === selectedColor)?.image || ""}
                            alt={selectedColor}
                            width={20}
                            height={20}
